...
- パラメータの適用機構は VideoEffectChain から ParameterBinder へ抽出し、obj.effect と obj.brush が同じ実装を共有します。プロパティ表のキャッシュは、エフェクトの適用が描画スレッド、模様の適用がスクリプトの実行スレッドで走るようになったため、ロックで保護しました。
