...
namespace LuaScript
{
    internal sealed class AviUtlScriptContext
    {
        ...
        internal IBufferCompositor Compositor { get; set; } = SoftwareCompositor.Instance;
        ...
        private double[]? _convolveSource;
        private byte[]? _resizeSource;
        private byte[]? _resizeBuffer;

        public unsafe void FillBuffer(double r, double g, double b, double a, int x, int y, int w, int h, bool force = false)
        {
            EnsurePixelBuffer();
            if (_pixelBuffer is null)
                return;
            ...
            int cw = ImageWidth;
            int ch = ImageHeight;
            int x0 = Math.Clamp(x, 0, cw);
            int y0 = Math.Clamp(y, 0, ch);
            int x1 = Math.Clamp(x + w, 0, cw);
            int y1 = Math.Clamp(y + h, 0, ch);
            if (x1 <= x0 || y1 <= y0)
                return;

            if (PixelProcessor?.TryFill(_pixelBuffer, cw, ch, r, g, b, a, x0, y0, x1 - x0, y1 - y0, force) == true)
            {
                _isPixelsDirty = true;
                return;
            }
        ...
        public unsafe void ReadRegion(int x, int y, int w, int h, double[] destination)
        {
            ...
            if (_pixelBuffer is null)
            {
                Array.Clear(destination, 0, w * h * 4);
                return;
            }
            ...
            fixed (byte* buf = _pixelBuffer)
            {
                for (int j = 0; j < h; j++)
                {
                    ...
                    for (int i = 0; i < w; i++)
                    {
                        int sx = x + i;
                        int di = (j * w + i) * 4;
                        if ((uint)sx >= (uint)cw || (uint)sy >= (uint)ch)
                        {
                            destination[di] = 0d;
                            destination[di + 1] = 0d;
                            destination[di + 2] = 0d;
                        ...
                        {
                            double s = 255d / a;
                            destination[di] = Math.Clamp(p[2] * s, 0d, 255d);
                            destination[di + 1] = Math.Clamp(p[1] * s, 0d, 255d);
                            destination[di + 2] = Math.Clamp(p[0] * s, 0d, 255d);
                            destination[di + 3] = a;
                        }
                    }
                }
        ...
        public unsafe void WriteRegion(int x, int y, int w, int h, double[] source)
        {
            EnsurePixelBuffer();
            if (_pixelBuffer is null)
                return;
                ...
                for (int j = 0; j < h; j++)
                {
                    ...
                    for (int i = 0; i < w; i++)
                    {
                        ...
                        if ((uint)dx >= (uint)cw)
                            continue;
                        int si = (j * w + i) * 4;
                        double a = Math.Clamp(source[si + 3], 0d, 255d);
                        double aK = a / 255d;
                        byte* p = buf + (dy * cw + dx) * 4;
                        p[0] = (byte)Math.Clamp(source[si + 2] * aK, 0d, 255d);
                        p[1] = (byte)Math.Clamp(source[si + 1] * aK, 0d, 255d);
                        p[2] = (byte)Math.Clamp(source[si] * aK, 0d, 255d);
                        p[3] = (byte)a;
                    }
                }
            }
        ...
        public unsafe void Convolve(double[] kernel, int size, double divisor, double offset, bool force = false)
        {
            EnsurePixelBuffer();
            if (_pixelBuffer is null)
                return;
            ...
            int cw = ImageWidth;
            int ch = ImageHeight;
            if (PixelProcessor?.TryConvolve(_pixelBuffer, cw, ch, kernel, size, divisor, offset, force) == true)
            {
                _isPixelsDirty = true;
                return;
            }
        ...
        public unsafe void Resize(int newWidth, int newHeight, bool linear, bool force = false)
        {
            EnsurePixelBuffer();
            if (_pixelBuffer is null)
                return;
            ...
            if (PixelProcessor?.TryResize(_pixelBuffer, cw, ch, newWidth, newHeight, linear, out var processed, force) == true && processed is not null)
            {
